...
function assetPath(path = '') {
  if (!path) return '../assets/hero.webp';
...
function renderRouteLinks(route) {
  ...
  const links = Array.isArray(route.links)
    ? route.links
    : [];

  wrap.innerHTML = links.map(link => `
    <a
      class="btn ${escapeHtml(link.style || 'btn-primary')}"
      href="${escapeHtml(linkPath(link.url))}"
      target="_blank"
      rel="noopener"
    >
...
function renderRouteSupport(route) {
  ...
  const items = Array.isArray(route.supportPoints)
    ? route.supportPoints
    : [];
    ...
    <section class="route-support reveal is-visible">
      ...
      <div class="grid-3 route-support-grid">
        ...
        ${items.map((item, index) => `
          <button
            class="card feature-item route-support-card"
            type="button"
...
function bindRouteSupportModal(items) {
  ...
  buttons.forEach(button => {
    ...
    button.addEventListener('click', () => {
      ...
      const item =
        items[Number(button.dataset.supportIndex)];
  ...
  modal?.addEventListener('click', e => {
    ...
    if (e.target.dataset.stageClose === 'true') {
      close();
    }
...
function bindStageModal(route) {
  ...
  const buttons =
    document.querySelectorAll('[data-stage-index]');
  ...
  const modalSubtitle =
    document.getElementById('stage-subtitle');

  const modalContent =
    document.getElementById('stage-content');

  const closeBtn =
    document.getElementById('stage-close-btn');
  ...
  buttons.forEach(button => {
    ...
    button.addEventListener('click', () => {
      ...
      modalTitle.textContent =
        `${stage.day || ''} — ${stage.route || ''}`;

      modalSubtitle.textContent =
        `${stage.distance || ''} • ${stage.summary || ''}`;
      ...
      modalContent.innerHTML = `
        ...
        <div class="stage-gallery">
          ...
          <div class="stage-gallery-carousel">
            <button class="route-nav prev" type="button" data-stage-gallery-prev aria-label="Foto anterior">
            ...
            <div class="stage-gallery-grid">
              ...
              ${(stage.photos || []).map((photo, idx) => `
                <button
                  ...
                  data-stage-open-photo="true"
                >
                  <img
                    src="${escapeHtml(assetPath(photo.src))}"
                    alt="${escapeHtml(photo.alt || stage.day || '')}"
                  >
                </button>
              `).join('')}
            ...
            <button class="route-nav next" type="button" data-stage-gallery-next aria-label="Próxima foto">
        ...
        <div class="modal-grid" style="margin-top:26px">

          <div class="support-block">
            <h4>🧭 Perfil da etapa</h4>

            <p>
              ${escapeHtml(stage.profile || '')}
            </p>
          </div>
          ...
          <div class="support-block">
            ...
            <p>
              ${escapeHtml(
                stage.attention ||
                'Confirme clima, hidratação e apoio local antes de sair.'
              )}
            </p>
        ...
        <div class="support-block" style="margin-top:24px">
      ...
      modal
        .querySelectorAll('[data-stage-open-photo]')
        .forEach(photoButton => {
          ...
          photoButton.addEventListener('click', () => {
            ...
            openPhotoModal(
              stage,
              Number(photoButton.dataset.photoIndex || 0)
            );
      ...
      setupStageGalleryCarousel(modal);
...
function setupStageGalleryCarousel(modal){

  const carousel =
    modal.querySelector('.stage-gallery-carousel');

  if(!carousel) return;

  const grid =
    carousel.querySelector('.stage-gallery-grid');

  const prev =
    carousel.querySelector('[data-stage-gallery-prev]');

  const next =
    carousel.querySelector('[data-stage-gallery-next]');

  const items = Array.from(
    carousel.querySelectorAll('[data-stage-open-photo]')
  );
...
function openPhotoModal(stage, startIndex = 0) {
  ...
  const counter =
    document.getElementById('photo-modal-counter');

  const thumbs =
    document.getElementById('photo-modal-thumbs');
    ...
    thumbs
      ...
      .forEach(button => {
        ...
        button.addEventListener('click', () => {
          ...
          currentIndex =
            Number(button.dataset.thumbIndex);
  ...
  modal.onclick = event => {
    ...
    if (event.target.dataset.photoClose === 'true') {
      hide();
    }
...
function renderPage(data, route) {
  ...
  document.title =
    `${route.title || 'Percurso'} | ${data.brand?.name || 'Rota do Ouro Verde'}`;

  const title =
    document.getElementById('route-title');

  const subtitle =
    document.getElementById('route-subtitle');

  const breadcrumb =
    document.getElementById('route-breadcrumb');

  const summary =
    document.getElementById('route-summary');

  const cover =
    document.getElementById('route-cover');
  ...
  const footerBrand =
    document.querySelector('[data-brand-name]');

  const whatsLinks =
    document.querySelectorAll('[data-brand-whatsapp]');
...
if (heroSignupBtn) {
  ...
  heroSignupBtn.href =
    `../inscricao/inscricao.html?route=${encodeURIComponent(route.id)}`;

  }

  if (breadcrumb) {
    breadcrumb.textContent =
      route.title || 'Percurso';
  }

  if (cover) {
    cover.style.backgroundImage =
      `url('${assetPath(route.detailCover || route.cover)}')`;
  }
      ...
      <div class="card route-panel reveal">
        ...
        <div class="route-actions" style="margin-top:18px">
          ...
          <a
            ...
            href="${escapeHtml(data.brand?.whatsapp || '#')}"
            target="_blank"
            rel="noopener"
          >
    ...
    if (!route.stages || !route.stages.length) {
      ...
      stageWrap.innerHTML = `
        <div class="empty-state">
          Este percurso ainda está em planejamento.
        </div>
      `;
      ...
      stageWrap.innerHTML = route.stages.map((stage, index) => `
        ...
        >
          ...
          <div
            class="stage-card-image"
            style="background-image:url('${escapeHtml(assetPath(stage.cover))}')"
          ></div>
          ...
          <div class="stage-card-content">
            ...
            <h3>
              ${escapeHtml(stage.day || '')}
            </h3>

            <p>
              <strong>
                ${escapeHtml(stage.route || '')}
              </strong>
            </p>

            <p>
              ${escapeHtml(stage.distance || '')}
              •
...
getSiteData()
  .then(data => {
    ...
    const route =
      routes.find(item => item.id === getParamId()) ||
      routes.find(item => item.featured) ||
      routes[0];

    if (!route) {
      throw new Error('Nenhum percurso encontrado');
    }
  ...
  .catch(error => {
    ...
    if (page) {
      ...
      page.innerHTML = `
        <div class="empty-state">
          Não foi possível abrir o percurso.
