...
function renderRouteLinks(route) {
  ...
  wrap.innerHTML = links.map(link => `
    <a class="btn ${escapeHtml(link.style || 'btn-primary')}" href="${escapeHtml(linkPath(link.url))}" target="_blank" rel="noopener">
          ...
          <div class="stage-gallery-grid">
            ${(stage.photos || []).map((photo, idx) => `
              <button type="button" data-photo-index="${idx}" data-stage-open-photo="true">
                <img src="${escapeHtml(assetPath(photo.src))}" alt="${escapeHtml(photo.alt || stage.day || '')}">
              </button>
            `).join('')}
          </div>
        </div>
...
function renderPage(data, route) {
  document.title = `${route.title || 'Percurso'} | ${data.brand?.name || 'Rota do Ouro Verde'}`;
  ...
  if (cover) {
    cover.style.backgroundImage = `url('${assetPath(route.cover)}')`;
  }
    ...
    summary.innerHTML = `
      <div class="card route-panel reveal">
        ...
        <div class="route-actions" style="margin-top:18px">
          <a class="btn btn-primary" href="${escapeHtml(data.brand?.whatsapp || '#')}" target="_blank" rel="noopener">
    ...
    if (!route.stages || !route.stages.length) {
      ...
      stageWrap.innerHTML = route.stages.map((stage, index) => `
        <button class="stage-card" type="button" data-stage-index="${index}">
          <div class="stage-card-image" style="background-image:url('${escapeHtml(assetPath(stage.cover))}')"></div>
